...
/*
TODO Bridge:
- onEvent: Need to preserve ordering of events / queuing. Provide options:
 ...
 * @param {string=} opts.queueType The type of queue to use when feeding through
 * to {@link Bridge~onEvent}. One of: "none", "per_room". If "none", you may
 * get events out of order. If "per_room", you will get guaranteed ordering per
 * room_id, but not between room IDs. Default: "none".
 */
function Bridge(opts) {
    ...
    opts.queueType = opts.queueType || "none";
    ...
    this._queue = new EventQueue(this.opts.queueType);
}
...
Bridge.prototype.run = function(port, config, appServiceInstance) {
    ...
    if (this.opts.controller.onLog) {
        this._requestFactory.addDefaultResolveCallback(function(req) {
...
Bridge.prototype._onEvent = function(event) {
    ...
    return promise.then(function() {
        self.opts.controller.onEvent(request, context);
    }).catch(function(e) {
        if (!self.opts.controller.onLog) {
            return;
        }
...
function EventQueue(queueType, consumeFn) {
    this.type = queueType;
    this._queues = {
        // $identifier: {
...
EventQueue.prototype.push = function(event, promise) {
...
EventQueue.prototype.consume = function() {
    Object.keys(this._queues).forEach(function(identifier) {
        if (!self._queues[identifier].consuming) {
            // TODO start consuming the queue
        }
    });
};
