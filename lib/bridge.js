...
Bridge.prototype._onUserQuery = function(userId) {
    ...
    if (self.opts.provisionUser) {
        ...
        return Promise.resolve(
            self.opts.provisionUser(userId, matrixUser.localpart)
        ).then(function(info) {
            ...
            promise.then(function() {
                ...
                if (info.user) {
                    storePromise.then(function() {
                        return self._userStore.linkUsers(matrixUser, info.user);
                    });
                }
                return storePromise;
            });
            ...
            if (info.name) {
                promise = promise.then(function() {
                    return newUser.setDisplayName(info.name);
                });
            }
            if (info.url) {
                promise = promise.then(function() {
                    return newUser.setAvatarUrl(info.url);
                });
            }
            return promise;
        });
 ...
 * @param {string} localpart The parsed user localpart from the user ID.
