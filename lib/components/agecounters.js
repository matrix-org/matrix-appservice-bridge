...
AgeCounters.prototype.bump = function(age) {
    if (age < HOUR) this.counters["1h"]++;
    if (age < DAY ) this.counters["1d"]++;
    if (age < WEEK) this.counters["7d"]++;
...
AgeCounters.prototype.setGauge = function(gauge, morelabels) {
    Object.keys(this.counters).forEach(function(age) {
        var labels = Object.assign({age: age}, morelabels);
        gauge.set(labels, this.counters[age]);
    });
};
