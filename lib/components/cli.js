...
Cli.prototype.run = function() {
    ...
    if (this.opts.enableRegistration && args["generate-registration"]) {
        if (!args.url) {
            this._printHelp();
            log.info("Missing --url [-u]");
            process.exit(1);
        }
        if (args.port) {
            this._printHelp();
            log.info("--port [-p] is not valid when generating a registration file.");
            process.exit(1);
        }
        if (this.opts.bridgeConfig && this.opts.bridgeConfig.affectsRegistration) {
            if (!args.config) {
                this._printHelp();
                log.info("Missing --config [-c]");
                process.exit(1);
            }
...
Cli.prototype._generateRegistration = function(appServiceUrl, localpart) {
    ...
    this.opts.generateRegistration.bind(this)(reg, function(completeReg) {
        ...
        log.info("Output registration to: " + self.opts.registrationPath);
        process.exit(0);
    });
};
...
Cli.prototype._printHelp = function() {
    ...
    log.info("Usage:\n");
    log.info("Generating an application service registration file:");
    log.info("%s %s\n", appPart, usages[0]);
    log.info("Running an application service with an existing registration file:");
    log.info("%s %s", appPart, usages[1]);
