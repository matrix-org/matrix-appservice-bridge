 ...
 * 
 * @param {Object=} opts.caching.ttl How long requests can stay in the cache, in milliseconds.
 */
function Intent(client, botClient, opts) {
    ...
    opts.caching.ttl = opts.caching.ttl === undefined ? DEFAULT_CACHE_TTL : opts.caching;
    this._requestCaches = {};
    this._requestCaches.profile = new ClientRequestCache(
        opts.caching.ttl,
        (_, userId, info) => {
        return this.intent.getProfileInfo(userId, info, false);
    });
    this._requestCaches.roomstate = new ClientRequestCache(
        opts.caching.ttl,
        (roomId) => {
        return this.intent.roomState(roomId, false);
    });
    this._requestCaches.event = new ClientRequestCache(
        opts.caching.ttl,
        (_, roomId, eventId) => {
        return this.intent.getEvent(roomId, false);
    });
 ...
 * @param {boolean=false} useCache Should the request attempt to lookup
 ...
 * @param {boolean=true} useCache Should the request attempt to lookup
 * state from the cache.
 ...
 * @param {boolean=true} useCache Should the request attempt to lookup from the cache.
