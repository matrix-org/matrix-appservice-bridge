...
class Logging {
    ...
    */
    Configure(config={}) {
        ...
        const formatterFn = function(opts, colors=false) {
            let level = opts.level.toUpperCase();
            const levelColour = CHALK_LEVELS[opts.level];
            if (colors && levelColour) {
                level = chalk[levelColour](level);
            }
        ...
        if (config.console !== undefined && config.console !== "off") {
            this.transports.push(new (winston.transports.Console)({
                ...
                formatter: (opts) => formatterFn(opts, true),
